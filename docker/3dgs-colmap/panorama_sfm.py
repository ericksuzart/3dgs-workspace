...
Patched for intra-rig matching in 3DGS Workspace:
  - JPEG quality 100 (lossless-like)
  - 8 yaw steps (24 virtual cameras per panorama)
  - Masks disabled → enables intra-rig matching
  - Matching of images in the same frame enabled
"""
...
class PanoProcessor:
    def __init__(
        ...
        self.mask_dir = mask_dir  # None = mask disabled for intra-rig matching
    ...
    def process(self, pano_name: str) -> None:
        ...
        for cam_idx, cam_from_pano_r in enumerate(self.cams_from_pano_rotation):
            ...
            PIL.Image.fromarray(image).save(
                image_path, exif=gpsonly_exif, quality=100, subsampling=0
            )
...
def run(args: argparse.Namespace) -> None:
    ...
    mask_dir = None  # mask disabled for intra-rig matching
    ...
    pycolmap.extract_features(
        database_path,
        image_dir,
        reader_options=pycolmap.ImageReaderOptions(),  # no mask
    ...
    # The images within a frame have overlap (masks disabled), so allow it.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override num_steps_yaw for the overlapping render type "
        "(default: 8). Reduces total virtual cameras to fit GPU memory. "
        "Also sets --remove-bottom in run.sh accordingly.",
    )
    args = parser.parse_args()
    if args.num_steps_yaw is not None:
        if args.pano_render_type not in PANO_RENDER_OPTIONS:
            logging.fatal(f"Unknown pano_render_type: {args.pano_render_type}")
        PANO_RENDER_OPTIONS[args.pano_render_type].num_steps_yaw = (
            args.num_steps_yaw
        )
    run(args)
